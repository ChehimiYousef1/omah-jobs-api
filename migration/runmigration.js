...
const migrations = [
  ...
  },
  {
    ...
    steps: [
      `IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'post_likes') BEGIN CREATE TABLE post_likes (id UNIQUEIDENTIFIER NOT NULL DEFAULT NEWID(), user_id UNIQUEIDENTIFIER NOT NULL, post_id UNIQUEIDENTIFIER NOT NULL, created_at DATETIME2 NOT NULL DEFAULT SYSDATETIME()); END`,
      `IF NOT EXISTS (SELECT 1 FROM sys.objects WHERE name = 'PK_post_likes') BEGIN ALTER TABLE post_likes ADD CONSTRAINT PK_post_likes PRIMARY KEY (id); END`,
      `IF NOT EXISTS (SELECT 1 FROM sys.objects WHERE name = 'FK_like_user') BEGIN ALTER TABLE post_likes ADD CONSTRAINT FK_like_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE NO ACTION; END`,
      `IF NOT EXISTS (SELECT 1 FROM sys.objects WHERE name = 'FK_like_post') BEGIN ALTER TABLE post_likes ADD CONSTRAINT FK_like_post FOREIGN KEY (post_id) REFERENCES Posts(id) ON DELETE CASCADE; END`,
      `IF NOT EXISTS (SELECT 1 FROM sys.objects WHERE name = 'UQ_like') BEGIN ALTER TABLE post_likes ADD CONSTRAINT UQ_like UNIQUE (user_id, post_id); END`,
      `IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_likes_post' AND object_id = OBJECT_ID('post_likes')) BEGIN CREATE NONCLUSTERED INDEX IX_likes_post ON post_likes(post_id) INCLUDE (user_id, created_at); END`,

      `IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'post_reposts') BEGIN CREATE TABLE post_reposts (id UNIQUEIDENTIFIER NOT NULL DEFAULT NEWID(), user_id UNIQUEIDENTIFIER NOT NULL, post_id UNIQUEIDENTIFIER NOT NULL, created_at DATETIME2 NOT NULL DEFAULT SYSDATETIME()); END`,
      `IF NOT EXISTS (SELECT 1 FROM sys.objects WHERE name = 'PK_post_reposts') BEGIN ALTER TABLE post_reposts ADD CONSTRAINT PK_post_reposts PRIMARY KEY (id); END`,
      `IF NOT EXISTS (SELECT 1 FROM sys.objects WHERE name = 'FK_repost_user') BEGIN ALTER TABLE post_reposts ADD CONSTRAINT FK_repost_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE NO ACTION; END`,
      `IF NOT EXISTS (SELECT 1 FROM sys.objects WHERE name = 'FK_repost_post') BEGIN ALTER TABLE post_reposts ADD CONSTRAINT FK_repost_post FOREIGN KEY (post_id) REFERENCES Posts(id) ON DELETE CASCADE; END`,
      `IF NOT EXISTS (SELECT 1 FROM sys.objects WHERE name = 'UQ_repost') BEGIN ALTER TABLE post_reposts ADD CONSTRAINT UQ_repost UNIQUE (user_id, post_id); END`,
      `IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_reposts_post' AND object_id = OBJECT_ID('post_reposts')) BEGIN CREATE NONCLUSTERED INDEX IX_reposts_post ON post_reposts(post_id) INCLUDE (user_id, created_at); END`,

      `IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'comments') BEGIN CREATE TABLE comments (id UNIQUEIDENTIFIER NOT NULL DEFAULT NEWID(), user_id UNIQUEIDENTIFIER NOT NULL, post_id UNIQUEIDENTIFIER NOT NULL, parent_id UNIQUEIDENTIFIER NULL, content NVARCHAR(MAX) NOT NULL, created_at DATETIME2 NOT NULL DEFAULT SYSDATETIME(), updated_at DATETIME2 NOT NULL DEFAULT SYSDATETIME()); END`,
      `IF NOT EXISTS (SELECT 1 FROM sys.objects WHERE name = 'PK_comments') BEGIN ALTER TABLE comments ADD CONSTRAINT PK_comments PRIMARY KEY (id); END`,
      `IF NOT EXISTS (SELECT 1 FROM sys.objects WHERE name = 'FK_comment_user') BEGIN ALTER TABLE comments ADD CONSTRAINT FK_comment_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE NO ACTION; END`,
      `IF NOT EXISTS (SELECT 1 FROM sys.objects WHERE name = 'FK_comment_post') BEGIN ALTER TABLE comments ADD CONSTRAINT FK_comment_post FOREIGN KEY (post_id) REFERENCES Posts(id) ON DELETE CASCADE; END`,
      `IF NOT EXISTS (SELECT 1 FROM sys.objects WHERE name = 'FK_comment_parent') BEGIN ALTER TABLE comments ADD CONSTRAINT FK_comment_parent FOREIGN KEY (parent_id) REFERENCES comments(id) ON DELETE NO ACTION; END`,
      `IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_comments_post' AND object_id = OBJECT_ID('comments')) BEGIN CREATE NONCLUSTERED INDEX IX_comments_post ON comments(post_id, created_at) INCLUDE (id, user_id, parent_id, content); END`,
      `IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_comments_parent' AND object_id = OBJECT_ID('comments')) BEGIN CREATE NONCLUSTERED INDEX IX_comments_parent ON comments(parent_id) INCLUDE (id, user_id, post_id, created_at); END`,

      `IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'post_interactions') BEGIN CREATE TABLE post_interactions (id UNIQUEIDENTIFIER NOT NULL DEFAULT NEWID(), post_id UNIQUEIDENTIFIER NOT NULL, user_id UNIQUEIDENTIFIER NOT NULL, type NVARCHAR(20) NOT NULL, created_at DATETIME2 NOT NULL DEFAULT SYSDATETIME()); END`,
      `IF NOT EXISTS (SELECT 1 FROM sys.objects WHERE name = 'PK_post_interactions') BEGIN ALTER TABLE post_interactions ADD CONSTRAINT PK_post_interactions PRIMARY KEY (id); END`,
      `IF NOT EXISTS (SELECT 1 FROM sys.objects WHERE name = 'CHK_pi_type') BEGIN ALTER TABLE post_interactions ADD CONSTRAINT CHK_pi_type CHECK (type IN ('like','comment','repost','view')); END`,
      `IF NOT EXISTS (SELECT 1 FROM sys.objects WHERE name = 'FK_pi_post') BEGIN ALTER TABLE post_interactions ADD CONSTRAINT FK_pi_post FOREIGN KEY (post_id) REFERENCES Posts(id) ON DELETE CASCADE; END`,
      `IF NOT EXISTS (SELECT 1 FROM sys.objects WHERE name = 'FK_pi_user') BEGIN ALTER TABLE post_interactions ADD CONSTRAINT FK_pi_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE NO ACTION; END`,
      `IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_pi_post_type' AND object_id = OBJECT_ID('post_interactions')) BEGIN CREATE NONCLUSTERED INDEX IX_pi_post_type ON post_interactions(post_id, type) INCLUDE (user_id, created_at); END`,
    ],
  },
  {
...
// ─── Runner (exported as a callable async function) ───────────────────────────
async function runMigrations() {
  ...
  try {
    ...
    for (const step of bootstrap.steps) await pool.request().query(step);
    ...
    for (const migration of migrations.slice(1)) {
      ...
      const t0 = Date.now();
      try {
        // Run each step directly — DDL with FILLFACTOR cannot run inside a transaction
        ...
        await pool.request()
          ...
          .input('ms', sql.Int,      ms)
          .query(`INSERT INTO migrations_log (version, description, duration_ms) VALUES (@v, @d, @ms)`);
