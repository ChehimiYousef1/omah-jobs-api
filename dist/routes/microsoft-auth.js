...
router.get('/microsoft/callback', async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            if (avatarRes.ok)
                fs.writeFileSync(avatarPath, Buffer.from(await avatarRes.arrayBuffer()));
        }
        catch (e) {
      ...
      `);
        ...
        const jwtToken = jwt.sign({ userId: user.id, email: user.email, role: user.role }, process.env.JWT_SECRET, { expiresIn: '7d' });
...
router.get('/me', async (req, res) => {
    try {
        ...
        if (!token)
            return res.status(401).json({ success: false, error: 'No token cookie' });
...
router.post('/update-cover', uploadCover.single('cover'), async (req, res) => {
    try {
        ...
        // 3️⃣ Check uploaded file
        if (!req.file)
            return res.status(400).json({ success: false, error: 'Cover file is required' });
