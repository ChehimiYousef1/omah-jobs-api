...
app.get('/api/auth/microsoft/callback', async (req, res) => {
  ...
  try {
    ...
    if (process.env.MICROSOFT_CLIENT_SECRET) bodyParams.client_secret = process.env.MICROSOFT_CLIENT_SECRET;
    ...
    if (!tokenData.access_token) return res.redirect(`${frontendUrl}/login?error=no_access_token`);

    const userRes = await fetch('https://graph.microsoft.com/v1.0/me', {
      headers: { Authorization: `Bearer ${tokenData.access_token}` },
    });
    if (!userRes.ok) return res.redirect(`${frontendUrl}/login?error=profile_fetch_failed`);
    ...
    if (!process.env.JWT_SECRET) return res.redirect(`${frontendUrl}/login?error=server_configuration_error`);
