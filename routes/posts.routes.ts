...
// ── Auth helper ───────────────────────────────────────────────────────────────
const getUser = (req: Request): { userId: string } => {
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    try { userId = getUser(req).userId; } catch { /* guest — no likes/reposts */ }
...
router.post('/', upload.single('file'), async (req: Request, res: Response) => {
  try {
    ...
    const { title = '', description = '', visibility = 'Anyone', postType = 'text',
            eventName, eventDate, eventTime, eventLocation, eventUrl } = req.body;
