...
// Helpers
// =========================
const redirectError = (res: express.Response, reason: string) =>
...
const saveAvatar = async (name: string, avatarUrl: string | null): Promise<string> => {
  ...
  try {
    ...
    if (res.ok) fs.writeFileSync(avatarPath, Buffer.from(await res.arrayBuffer()));
  } catch (e) {
...
const upsertUser = async (email: string, name: string, avatar: string) => {
...
const signAndSetCookie = (res: express.Response, user: any) => {
...
router.get('/github/callback', async (req, res) => {
  try {
    ...
    if (!stateStore.has(state)) return redirectError(res, 'invalid_state');
    stateStore.delete(state);

    const accessToken = await exchangeCodeForToken(code);
    if (!accessToken) return redirectError(res, 'token_exchange_failed');

    const ghUser = await fetchGitHubUser(accessToken);
    if (!ghUser)      return redirectError(res, 'user_fetch_failed');

    const email = ghUser.email ?? await fetchGitHubEmail(accessToken);
    if (!email)       return redirectError(res, 'email_missing');
